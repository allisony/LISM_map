...
if True:
    ...
    mask = d>10#(d>30) & (d <= 50)
    filename_precursor = 'all_outside_10pc_leaveoneout_'
...
if run_loop:
    ...
    q_array = np.zeros((len(y_obs)),int(len(phi)*len(theta))))
    ...
    for i in range(len(y_obs)):
        ...
        y_obs_tmp = np.delete(y_obs,i)
        X_obs_tmp = np.delete(X_obs,i,axis=0)
        yerr_tmp = np.delete(yerr,i)
        ...
        mcmc.run(rng_key, X_obs_tmp, yerr_tmp, y=y_obs_tmp)
    ...
    q_min = np.min(q_array,axis=0)
    q_max = np.max(q_array,axis=0)
...
unc = np.mean([q[1]-q_min,q_max-q[1]],axis=0)
#######################################################
...
if True:
    ...
    np.savetxt('NHI_column_map_minmax_uncertainty_'+filename_precursor +'.txt',np.transpose(unc))
