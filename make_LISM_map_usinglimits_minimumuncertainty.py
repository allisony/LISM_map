...
if False:
    ...
    df['N(HI) uncertainty adjusted'] = 0

    high_res_gratings = ['GHRS/ECH-A', 'STIS/E140H', 'STIS/E140M']
    low_res_gratings = ['GHRS/G140M', 'GHRS/G160M', 'STIS/G140M']
    high_res_minimum = 0.10 # dex
    low_res_minimum = 0.30 # dex

    df.loc[(df['Instrument/Grating'].isin(high_res_gratings)) & (df['N(HI) uncertainty'] <= high_res_minimum), 'N(HI) uncertainty adjusted'] = high_res_minimum
    df.loc[(df['Instrument/Grating'].isin(high_res_gratings)) & (df['N(HI) uncertainty'] > high_res_minimum), 'N(HI) uncertainty adjusted'] = df['N(HI) uncertainty']
    df.loc[(df['Instrument/Grating'].isin(high_res_gratings)) & (df['N(HI) uncertainty'] == 0), 'N(HI) uncertainty adjusted'] = high_res_minimum

    df.loc[(df['Instrument/Grating'].isin(low_res_gratings)) & (df['N(HI) uncertainty'] <= low_res_minimum), 'N(HI) uncertainty adjusted'] = low_res_minimum
    df.loc[(df['Instrument/Grating'].isin(low_res_gratings)) & (df['N(HI) uncertainty'] > low_res_minimum), 'N(HI) uncertainty adjusted'] = df['N(HI) uncertainty']
    df.loc[(df['Instrument/Grating'].isin(low_res_gratings)) & (df['N(HI) uncertainty'] == 0), 'N(HI) uncertainty adjusted'] = low_res_minimum

    #### NEED TO TREAT ZEROS / EMPTIES!!!

    #df.loc[df['N(HI) source'].isin(['Wood2005']), 'N(HI) uncertainty'] = high_res_minimum

    df.to_csv('targets/NHI_data_August2025_fixed.csv')
...
skycoords2 = SkyCoord(ra = skycoords.galactic.l.value * u.degree, dec = skycoords.galactic.b.value * u.degree) ## Make it think the l,b coordinates are RA, Dec #####

X_obs_all = np.array(skycoords2.cartesian.xyz.T) # shape (100,3) -- for unit vectors
theta_obs_all = np.asarray(skycoords2.icrs.dec) * np.pi/180. 
phi_obs_all  =  np.asarray(skycoords2.icrs.ra) * np.pi/180.
####################################################


## Set up data to fit ##############################
y_obs_all = df['N(HI)'].values
yerr_all= df['N(HI) uncertainty adjusted'].values
...
shell_params = {'all_inside_10pc': [0,10],
                '10_20pc': [10, 20],
                '20_30pc': [20, 30],
                '30_50pc': [30, 50],
                '50_70pc': [50, 70],
...
if True:
    ...
    filename_precursor = '70_100pc' # EDIT
    dmin = shell_params[filename_precursor][0]
    dmax = shell_params[filename_precursor][1]
      

    mask = (d_all>dmin) & (d_all <= dmax) 
...
def numpyro_model(X_obs, yerr, y=None, X_obs_upper=None, y_upper=None, yerr_upper = None, X_obs_lower=None, y_lower=None, yerr_lower=None): 
    ...
    if X_obs_upper is not None and y_upper is not None and yerr_upper is not None:
        ...
        upper_limit_likelihood = jax.nn.sigmoid(sigmoid_factor*upper_distance)
        #print("upper")
    ...
    if X_obs_lower is not None and y_lower is not None and yerr_lower is not None:
        ...
        lower_distance = (mu_lower + jnp.sqrt(var_lower)) - (y_lower - yerr_lower)
        lower_limit_likelihood = jax.nn.sigmoid(sigmoid_factor*lower_distance)
        #print("lower")
...
if True:
    ...
    np.savetxt('Aug2025/NHI_column_map_'+filename_precursor +'_upperlowerlimits.txt',np.transpose(q))
    np.savetxt('Aug2025/NHI_column_fitted_stars_' + filename_precursor + '_upperlowerlimits.txt',np.transpose(np.array([phi_obs,theta_obs,y_obs])))
    table.to_csv('Aug2025/Bestfit_hyperparameters_' + filename_precursor + '_upperlowerlimits.csv')
#######################################################################
